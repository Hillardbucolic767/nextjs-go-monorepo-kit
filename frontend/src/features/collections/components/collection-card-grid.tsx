  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-5">
      {collections.map((collection) => (
        <Card key={collection.id} className="rounded-[24px] border border-white/60 bg-white/80 p-4 shadow-[var(--shadow)]">
          <div className="mb-4 aspect-[4/3] overflow-hidden rounded-[16px]">
            <PlaceholderArt title={collection.title} label={collection.category} tone="soft" />
          </div>
          <CardContent className="p-0">
            <h3 className="mb-1 text-[1.15rem]">{collection.title}</h3>
            <p className="mb-4 text-[var(--muted-text)]">{collection.description}</p>
