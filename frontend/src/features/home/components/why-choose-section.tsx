    ...
    <section className="px-0 pt-12">
      <div className="mx-auto grid w-[min(var(--max-width),calc(100%-2rem))] gap-6 rounded-[32px] border border-white/70 bg-white/75 p-[clamp(1.5rem,4vw,2rem)] shadow-[var(--shadow)] max-[720px]:w-[min(var(--max-width),calc(100%-1.25rem))]">
        <div className="section-heading">
          ...
          <p>
            Every section is designed to be a reusable example you can reshape
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-4">
          {whyChooseUs.map((feature) => (
            ...
            >
              <span className="inline-flex h-[52px] w-[52px] items-center justify-center rounded-2xl bg-[var(--brand-soft)] text-2xl">
                {feature.icon}
              </span>
              <h3 className="mt-4 mb-2 text-[1.17rem]">{feature.title}</h3>
